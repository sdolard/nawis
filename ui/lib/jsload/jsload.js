...
JSLoad.prototype.loadScript = function(srcSetObj, iteration){
	...
	function loadNext(finalization){
	...
	if (srcSetObj.srcToLoad.length === 0) {
		...
		if (this.scriptConcatenatorURL) {
			...
			createScriptEl(this.scriptConcatenatorURL +
			srcSetObj.srcToLoad.join(",") +
			(srcSetObj.version ? "&version=" + srcSetObj.version : ""), srcSetObj);
			...
			this.sourceFilesLoaded[url] = true;
			createScriptEl(url + (srcSetObj.version ? "?version=" + srcSetObj.version : ""), srcSetObj, iteration + 1);
		}
	}
};
